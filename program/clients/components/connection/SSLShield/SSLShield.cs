namespace Zealot.client.connection
{
    public sealed class SSLShield : _SSLShield.Controller
    {
        void Construction()
        {
            ...
            {
                listen_message<Client.IEndInitialize.SSLConnection, IReceive>(BUS.Client.Message.INITIALIZE)
                    .output_to(Initialize);

                listen_message<IConnection>(BUS.Content.Message.SUCCESS_CONNECTION)
                    .output_to(SuccessConnection);

                listen_impuls(BUS.Content.Impuls.ERROR_CONNECTION)
                    .output_to(ErrorConnection);
            }
